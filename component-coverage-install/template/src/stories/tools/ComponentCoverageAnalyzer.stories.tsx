...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          "上傳 UI 圖片或輸入產品功能文字（PRD），再將通用分析提示詞貼到 Cursor、Claude Code 或 Codex；agent 會使用 component-coverage-analyze skill 比對 `componentCatalog.ts` 與元件原始碼後產出覆蓋度報告：可直接使用、需擴充 variant、缺少需新建。完成覆核後，可將同樣 agent-neutral 的實作提示詞交給任一 agent，透過 component-coverage-implement skill 接續開發。請求與報告契約定義於 `src/storybook/component-coverage/coverageTypes.ts`。",
      },
    },
    layout: "fullscreen",
  },
