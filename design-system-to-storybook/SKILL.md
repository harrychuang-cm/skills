...
4. Record an implementation map before code changes. Prefer `design-system/STORYBOOK_IMPLEMENTATION_MAP.md` when the design-system package lives in the product repo; otherwise use `docs/design-system/storybook-implementation.md`.
5. Install and configure the Figma export addon when the product has a compatible React Storybook 10 setup.
6. If the component inventory has more than 8 implementable items, create or update a component queue before reading every spec or editing code.
7. If the product has explicit design-system governance instructions, follow them. Otherwise apply the gates in this skill.
...
### 3. Figma Export Addon

Install and configure `@harrychuang/storybook-addon-figma-export` by default when all requirements are met:
...
The addon is installed from GitHub:

```sh
npm install -D "github:harrychuang/storybook-addons#main:packages/figma-export" @storybook/icons
```

Use the equivalent package-manager command:

```sh
pnpm add -D "github:harrychuang/storybook-addons#main:packages/figma-export" @storybook/icons
yarn add -D "github:harrychuang/storybook-addons#main:packages/figma-export" @storybook/icons
bun add -d "github:harrychuang/storybook-addons#main:packages/figma-export" @storybook/icons
```
...
5. Record the installed package, config files, and options in the implementation map.
...
| Batch | Components | Dependencies | Target files | Validation | Status |
|---|---|---|---|---|---|
| `B01` | component names | tokens/components needed first | planned product files | checks to run | queued/done/blocked |
...
2. Search for an existing shared component with matching purpose, anatomy, behavior, and states.
3. Prefer reuse or extension over creating a new component.
4. Implement props, slots, variants, states, accessibility behavior, and responsive behavior from the spec.
5. Keep component styles token-backed. Do not reach directly into reference tokens from component CSS unless the extracted architecture explicitly allows it.
6. Export the component through the repo's existing public API.
...
- Figma export addon source: `https://github.com/harrychuang/storybook-addons/tree/main/packages/figma-export`.
