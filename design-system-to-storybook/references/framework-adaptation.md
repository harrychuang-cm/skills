...
| Bundled Figma export addon | Verified auto-install baseline is React + Storybook 10; its current React element/types and peer dependencies mean other renderers are not automatically validated | Auto-install only on the verified baseline. For another renderer, require compatibility evidence or an approved validation pass; otherwise mark unavailable/blocked without blocking core Storybook work. |
