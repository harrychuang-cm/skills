import type { VisualMeeting, VisualMeetingFile } from "./visualCommentStore";
...
export function renderVisualCommentReport(meeting: VisualMeetingFile): string {
  ...
  const sections = captures
    .map((capture) => {
      ...
      const pins = comments
        .map(
          (comment, index) =>
            `<span class="pin" style="left:${comment.pin.xRatio * 100}%;top:${comment.pin.yRatio * 100}%">${index + 1}</span>`,
        )
        .join("");
      const cards = comments
        .map(
          (comment, index) =>
            `<article><strong>${index + 1}. ${escapeHtml(comment.authorName)}</strong><time>${escapeHtml(comment.createdAt)}</time><p>${escapeHtml(comment.body)}</p></article>`,
        )
        .join("");
      ...
      const metadata = [capture.story.routeId, capture.story.stateId]
        .filter(Boolean)
        .map(escapeHtml)
        .join(" · ");
...
export function renderVisualCommentIndex(
  meetings: VisualMeeting[],
  activeSessionId: string | null,
): string {
  const links = [...meetings]
    .sort((a, b) => b.startedAt.localeCompare(a.startedAt))
    .map(
      (meeting) =>
        `<li><a href="sessions/${encodeURIComponent(meeting.id)}/index.html">${escapeHtml(meeting.title)}</a> <small>${escapeHtml(meeting.startedAt)}</small>${meeting.id === activeSessionId ? " (active)" : ""}</li>`,
    )
    .join("");
