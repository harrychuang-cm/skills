...
function renderVisualCommentReport(meeting, context = {
  ...
  const evidence = captures.map((capture) => {
    ...
    const pins = comments.map(
      (comment, index) => `<span class="pin" aria-label="Comment ${index + 1}" style="left:${comment.pin.xRatio * 100}%;top:${comment.pin.yRatio * 100}%">${index + 1}</span>`
    ).join("");
    const cards = comments.length ? comments.map((comment, index) => {
      ...
      return `<article class="comment" data-comment-card data-comment-status="${completed ? "completed" : "open"}" data-comment-endpoint="${escapeHtml(endpoint)}"><div class="comment__meta"><div class="comment__identity"><strong>${index + 1}. ${escapeHtml(comment.authorName)}</strong><span class="comment__status${completed ? " comment__status--completed" : ""}">${completed ? "Completed" : "Open"}</span></div><time>${escapeHtml(comment.createdAt)}</time></div><p class="comment__body">${escapeHtml(comment.body)}</p><script type="application/json" class="ai-fix-context" data-ai-fix-context>${htmlSafeJson(portableContext)}</script><div class="comment__actions"><button type="button" class="comment__action" data-comment-action="copy-ai-prompt">Copy AI prompt</button><button type="button" class="comment__action comment__action--delete" data-comment-action="delete" aria-label="Delete comment" title="Delete comment">${trashIcon}</button><button type="button" class="comment__action comment__action--primary" data-comment-action="resolve">${completed ? "Reopen" : "Complete"}</button></div><p class="comment__copy-status" data-ai-copy-status aria-live="polite" hidden></p><p class="comment__error" data-comment-error aria-live="polite" hidden></p></article>`;
...
function renderVisualCommentIndex(meetings, activeSessionId) {
  ...
  const active = sorted.find((meeting) => meeting.id === activeSessionId);
  const closed = sorted.filter((meeting) => meeting.id !== activeSessionId);
  return documentShell(
    "Visual review meetings",
    `<div class="topline"><span class="eyebrow">Figma export review</span></div><h1>Visual review meetings</h1><p class="summary">Current work and durable closed-session evidence.</p><section class="group"><h2>Current meeting</h2><div class="meeting-grid">${active ? meetingCard(active, true) : '<p class="empty">No active meeting.</p>'}</div></section><section class="group"><h2>Closed meeting history</h2><div class="meeting-grid">${closed.length ? closed.map((meeting) => meetingCard(meeting, false)).join("") : '<p class="empty">No closed meetings yet.</p>'}</div></section>`
  );
}
...
async function handleVisualCommentsRequest({
  ...
  try {
    ...
    if (segments[2] === "comments" && segments[3] && segments.length === 4) {
      ...
      if (method === "PATCH") {
        const requestBody = await readRequestBodyLimited(request);
        if (!isRecord(requestBody) || Object.keys(requestBody).length !== 1 || typeof requestBody.resolved !== "boolean") {
