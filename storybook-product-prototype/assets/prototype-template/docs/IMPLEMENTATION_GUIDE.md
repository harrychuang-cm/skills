...
5. Draft `PRODUCTION_HANDOFF.md` to map prototype behavior to web/app frontend implementation work.
6. Compose the interactive surface in `__FEATURE_PASCAL__.tsx`.
7. Add only prototype shell styles to `__FEATURE_KEBAB__.css`.
8. Import all documents and flow metadata in `__FEATURE_CAMEL__Meta.ts`.
9. Add `figmaExport.flowStoryId` in `__FEATURE_CAMEL__Meta.ts`.
10. Attach the meta object to `parameters.prototype` in `__FEATURE_PASCAL__.stories.tsx`.
11. Create `__FEATURE_PASCAL__FlowExport.tsx` and `__FEATURE_PASCAL__FlowExport.stories.tsx` with `StaticFlow`.
12. Confirm `prototypeRoute`, `prototypeFlowPreview`, `data-prototype-route-preview`, and `data-prototype-root` support UI Flow iframe previews.
13. Verify Storybook, docs, frontend handoff, UI Flow metadata, Static Flow export, and TypeScript.
