...
4. **Implement components first, then compose**
   ...
   - When at least one component work item exists, load both `$design-system-to-storybook` and `$ds-governance` **before any component mutation**. Resolve `$ds-governance` from the explicitly selected cm-skills checkout or the host's skill catalog; a direct read of `<cm-skills checkout>/ds-governance/SKILL.md` is sufficient. If either companion cannot be loaded, stop the affected component mutations and report which companion skill must be installed or made discoverable. Reuse already approved report decisions only for their named component scope; do not ask for the same approval again. Do not fall back to creating or extending components from these instructions alone.
